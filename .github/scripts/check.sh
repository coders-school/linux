...
pattern ${FILE} "'ps aux > $P' used" "ps(\s+)[-]?aux(\s*)>(\s*)$P"
...
pattern ${FILE} "Grepped for '/bin' in '$P' and appended to $S" "grep(\s+)-.*n.*\/bin(\s+)$P(\s+)>>(\s+)$S"
pattern ${FILE} "'history' with 'tail' used and saved to $M" "history(\s+)\|(\s+)tail(\s+)-n(\s*)\d+(\s+)>{1,2}(\s+)$M"
